...
export const fetchFieldNotes = createAsyncThunk('TaskManage/GetFieldNotes', async (id, { rejectWithValue }) => {
   try {
       const res = await axios.get(`/api/TaskManage/GetTask/${id}`);
...
export const createFieldNote = createAsyncThunk('FieldNotes/CreateFieldNote', async (fieldNoteData, { rejectWithValue }) => {
    try {
        const res = await axios.post('/api/TaskManage/CreateTask', fieldNoteData, {
            headers: {
        ...
        return rejectWithValue(error.response?.data?.error || 'Failed to create field note');
    }
});
...
const FieldNotesSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(createFieldNote.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(createFieldNote.fulfilled, (state, action) => {
                state.status = 'succeeded';
                state.FieldNotes.push(action.payload);
            })
            .addCase(createFieldNote.rejected, (state, action) => {
                state.status = 'failed';
